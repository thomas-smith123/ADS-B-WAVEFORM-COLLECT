...
board_read::board_read(sharedsources* sharedresource,QObject *parent)
    : QObject{parent}
{
...
void board_read::config()
{
    ...
    rxcfg.lo_hz = GHZ(1.092); // 2.5 GHz rf frequency
...
void board_read::start_read()
{
    ...
    if(start_flag)
    {
        ...
        while (!board_read::stop_)
        {
            ...
            if (sharedresources->emit_signal_to_process)
            {
                ...
                emit read_onece_done(I,Q,1024*1024,10e6);
            }
    ...
    if(start_flag)
    {
        while (!board_read::stop_)
        {
            ...
            if (emit_signal_to_process)
            {
                for (int var = 0; var < 1024*1024; ++var)
                {
                    {
                        I[var] = e1();
                        Q[var] = e1();
...
void board_read::errchk(int v, const char* what) {
    if (v < 0) { fprintf(stderr, "Error %d writing to channel \"%s\"\nvalue may not be supported.\n", v, what); shutdownDevice(); }
}

/* write attribute: long long int */
void board_read::wr_ch_lli(struct iio_channel* chn, const char* what, long long val)
{
    errchk(iio_channel_attr_write_longlong(chn, what, val), what);
}

/* write attribute: string */
void board_read::wr_ch_str(struct iio_channel* chn, const char* what, const char* str)
{
